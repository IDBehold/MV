...
\@x{}\moduleLeftDash\@xx{ {\MODULE} SimpleMath}\moduleRightDash\@xx{}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
This is a TLA+ module that asserts some simple mathematical formulas
 to be true. Each formula is preceded by the TLA+ keyword
 \ensuremath{{\ASSUME}},
 which means that it is to be taken as an assumption. \ensuremath{TLC} checks
 that
...
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
You can modify this file and run \ensuremath{TLC} to check your own formulas.
...
\begin{cpar}{1}{T}{T}{2.5}{2.5}{%
-}%
In your formulas, you can use natural numbers (0, 1, 2, \.{\dots} ),
 strings like ``abc'', and the values a, \ensuremath{b}, \ensuremath{c},
 \ensuremath{d}, \ensuremath{e}, \ensuremath{f}, and \ensuremath{g}, which
 \ensuremath{TLC} will interpret as arbitrary values that are unequal to each
 other and to any other value.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{T}{T}{2.5}{2.5}{%
-}%
Use only bounded quantifiers; \ensuremath{TLC} cannot handle the unbounded
 quantifiers. (It also cannot handle the unbounded \ensuremath{{\CHOOSE}}
 operator).
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
 This file contains a number of \ensuremath{{\ASSUME}} statements. They could
 be
 replaced by a single \ensuremath{{\ASSUME}} that assumes the conjunction of
 all the
 formulas, but using separate \ensuremath{ASSUMEs} makes it easier to locate
 an
 error.
\end{cpar}%
\vshade{5.0}%
...
Note: Table 8 tells you how to type all the symbols that do not have
 obvious \ensuremath{ASCII} equivalents.
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANTS} a ,\, b ,\, c ,\, d ,\, e ,\, f ,\, g}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This statement declares the values a, \.{\dots} , \ensuremath{g} so they can
 be used in
 formulas.
\end{cpar}%
\end{lcom}%
...
\end{lcom}%
\@x{ {\ASSUME}}%
 \@x{\@s{8.2} \A\, F ,\, G \.{\in} \{ {\TRUE} ,\, {\FALSE} \} \.{:} ( F
 \.{\implies} G ) \.{\equiv} {\lnot} F \.{\lor} G}%
\@pvspace{16.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{lcom}%
\@x{ {\ASSUME}}%
 \@x{\@s{8.2} {\lnot} \A\, F ,\, G \.{\in} \{ {\TRUE} ,\, {\FALSE} \} \.{:} (
 F \.{\lor} G ) \.{\implies} ( F \.{\land} G )}%
\@pvspace{16.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{lcom}%
\@x{ {\ASSUME}}%
 \@x{\@s{8.2} \{ 1 ,\, 2 ,\, 2 ,\, 3 ,\, 3 ,\, 3 \} \.{=} \{ 3 ,\, 1 ,\, 1 ,\,
 2 \}}%
\@pvspace{8.0pt}%
\@x{ {\ASSUME}}%
 \@x{\@s{8.2} \{ 1 ,\, 2 \} \.{\cup} \{ 2 ,\, 3 ,\, 4 \} \.{=} \{ 5 ,\, 4 ,\,
 3 ,\, 2 ,\, 1 \} \.{\cap} \{ 1 ,\, 2 ,\, 3 ,\, 4 \}}%
\@pvspace{8.0pt}%
\@x{ {\ASSUME}}%
\@x{\@s{8.2} \{ 1 ,\, 3 \} \.{\subseteq} \{ 3 ,\, 2 ,\, 1 \}}%
\@pvspace{8.0pt}%
\@x{ {\ASSUME}}%
 \@x{\@s{8.2} \{ a ,\, b ,\, c \} \.{\,\backslash\,} \{ c \} \.{=} \{ a ,\, b
 \}}%
\@pvspace{8.0pt}%
\@x{ {\ASSUME}}%
 \@x{\@s{8.2} \{ a ,\, b \} \.{\in} \{ \{ a ,\, b \} ,\, c ,\, \{ d ,\, e \}
 \}}%
\@pvspace{16.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The following defines \ensuremath{SomeSets} to be the set of all subsets of
 the set
 \ensuremath{\{a,\, b,\, c,\, d,\, e\}}. The \ensuremath{{\ASSUME}} that
 follows shows how you can use this
 set to have \ensuremath{TLC} check that a property of sets hold for all the
 sets in
 \ensuremath{SomeSets}. (This doesn\mbox{'}t imply that the property is valid
 for all sets,
 but it\mbox{'}s likely to discover if the property is not valid.)
\end{cpar}%
\end{lcom}%
\@x{ SomeSets \.{\defeq} {\SUBSET} \{ a ,\, b ,\, c ,\, d ,\, e \}}%
\@pvspace{8.0pt}%
\@x{ {\ASSUME}}%
 \@x{\@s{8.2} \A\, S ,\, T \.{\in} SomeSets \.{:} ( S \.{\subseteq} T )
 \.{\equiv} S \.{=} ( S \.{\cap} T )}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\end{document}
